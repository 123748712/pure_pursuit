...
class pure(Node):
    def __init__(self):
        ...
        self.subscription_pose = self.create_subscription(PoseWithCovarianceStamped, '/amcl_pose', self.pose_callback, 10)
        self.subscription_scan = self.create_subscription(LaserScan, '/scan', self.scan_callback, 10)
        # self.subscription_clicked = self.create_subscription()
    ...
    def pose_callback(self, msg):
        ...
        if self.value_setting:
            self.set_parameters([
                rclpy.parameter.Parameter("goal_x", rclpy.Parameter.Type.DOUBLE, self.current_x),
                rclpy.parameter.Parameter("goal_y", rclpy.Parameter.Type.DOUBLE, self.current_y)
            ])
            self.value_setting = False
